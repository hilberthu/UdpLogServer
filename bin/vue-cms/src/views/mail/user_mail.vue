<template>
  <div class="block">
    ...
    <div class="demo-input-suffix">
      玩家UID：
      <el-input type="textarea" :rows="5" placeholder="请输入UID，多个玩家以;分隔" v-model="uid_list"></el-input>
    </div>
    <el-divider></el-divider>
...
function add_user_callback(vueobj, response) {
...
export default {
  ...
  methods: {
    ...
    Send() {
      ...
      SendData.UidList = this.uid_list.split(";");
      for (var index in SendData.UidList) {
        if (
          String(parseInt(SendData.UidList[index])) != SendData.UidList[index]
        ) {
          alert("UID格式错误，请检查！");
          return;
        }
      ...
      Utils.myajax("add_user_mail", strSendData, add_user_callback, this);
      console.log("SendData", strSendData);
    }
  }
};
</script>